...
function DataCard({type, title, subTitle, tailData, style }) {
  ...
  return (
    <div className={`bg-white py-2 rounded-xl 2xl:ps-16 px-4 h-${hight ? hight : 'full'} w-full  hover:bg-slate-200`}>
      <div className='grid grid-cols-11 h-full w-full'>
        <div className={`col-span-3 h-full ${dots && 'flex items-center'}`}>
          ...
          <div className='w-full h-full flex justify-center items-center'>
            < img className='h-12 sm:h-12 md:h-14 lg:h-16 rounded-full' src={imageSrc} alt="" />
          </div>
          )}
        </div>
        <div className='col-span-5 h-full flex flex-col gap-0 justify-center items-center'>
          <h1 className='font-semibold text-md sm:text-md md:text-xl'>{title}</h1>
          {subTitle && <h2 className='text-[#666666] text-sm sm:text-md md:text-lg '>{subTitle}</h2>}
        </div>
        <div className='col-span-3  h-full flex justify-center items-center'>
          <h1 className='font-semibold text-sm sm:text-md md:text-xl'>{tailData}</h1>
        </div>
      </div>
