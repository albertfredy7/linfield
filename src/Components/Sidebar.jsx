...
function Sidebar() {
   ...
   const navigationItems = [
      ...
      },
      {
         to: '/expense',
         icon: (
            <AttachMoneyIcon fontSize={isTabletScreen ? 'large' : 'medium'} />
         ),
      ...
      },
      {
         ...
         icon: (
            <SensorOccupiedIcon
               fontSize={isTabletScreen ? 'large' : 'medium'}
            />
         ),
      ...
      <div className="h-screen p-5 overflow-y-hidden fixed">
         <div className="w-full h-full rounded-xl lg:flex lg:flex-col lg:justify-between bg-white ">
            <div className="p-6 h-full">
               ...
               <div
                  className={`mt-${
                     isTabletScreen ? '20' : '16'
                  } flex flex-col ${
                     isTabletScreen ? 'gap-4 justify-between' : 'gap-36'
                  }`}
               >
                  <div className="font-normal flex-grow">
                     <ul
                        className={`flex justify-center items-center flex-col ${
                           isTabletScreen ? 'mt-10' : 'mt-10'
                        }`}
                     >
                        {navigationItems.map((item) => (
                           <li key={item.to} className="mb-2 w-full">
                              <Link
                                 ...
                                 } space-x-2 rounded-3xl px-10 p-2 ${
                  ...
                  <button
                     className={`flex start space-x-2 rounded-3xl px-10 p-2 ${
                        isTabletScreen ? 'mt-50' : ''
                     }`}
                  >
                     <span className="flex items-center justify-center gap-3 text-xl">
                        <LogoutIcon
