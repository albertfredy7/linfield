...
function Sidebar() {
    ...
    return (
        <div className='h-screen p-5'>
            <div className="w-68 h-full rounded-xl lg:flex lg:flex-col lg:justify-between bg-white">
                <div className="p-6">
                    <div className="flex flex-row justify-around md:gap-2 lg:gap-2 xl:gap-0">
                        ...
                        {!isTabletScreen && (
                            <div className="flex flex-col justify-center items-start gap-0">
                                <h2 className="text-md text-[#666666]">Welcome back !</h2>
                                <h1 className="text-lg font-bold">Nishad 👋</h1>
                            </div>
                        )}
                    </div>
                    <div className={` h-full mt-${isTabletScreen ? '20' : '16'} flex flex-col ${isTabletScreen ? 'gap-4 justify-between items-center' : 'gap-8 items-center'}`}> 
                            ...
                            <ul className={`flex justify-center items-center flex-col ${isTabletScreen ? 'mt-10' : 'mt-10'}`}>
                                {navigationItems.map((item) => (
                                    <li key={item.to} className="mb-1 w-full text-center">
                                        <Link to={item.to} className={`flex ${isTabletScreen ? 'justify-center' : 'start'} space-x-2 rounded-3xl p-2 ${location.pathname === item.to ? 'bg-[#5266D7] text-white' : ''}`}>
                        ...
                        <div className="h-full w-full lg:flex lg:flex-col lg:justify-end">
                            <button className={`flex space-x-2 rounded-3xl p-2 ${isTabletScreen ? 'mt-50' : '16'}`}>
