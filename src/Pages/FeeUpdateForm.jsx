...
function FeeUpdateForm() {
   ...
   useEffect(() => {
      const fetchStudent = async () => {
         ...
         //  const { data } = await axios.get(
         //     `http://127.0.0.1:5000/api/students/search/${number}`
         //  );

         const { data } = await axios.get(
            `https://lms-backend-0hls.onrender.com/api/students/search/${number}`
         );
   ...
   useEffect(() => {
      const changeAmount = () => {
         ...
         } else if (studentData && feeType === 'examFees') {
            setAmount(studentData.feeDetails.examFees);
         } else if (studentData && feeType === 'registrationFees') {
            setAmount(studentData.feeDetails.registrationFees);
         } else {
