...
function UpdateFee() {
      ...
      try {
         ...
         if (Array.isArray(data)) {
            ...
            if (data.length > 0) {
               ...
               //  const transactionsResponse = await axios.get(
               //     `http://127.0.0.1:5000/api/students/studentTransactions/${admissionNumber}`
               //  );

               const transactionsResponse = await axios.get(
                  `https://lms-backend-0hls.onrender.com/api/students/studentTransactions/${admissionNumber}`
               );
            ...
            if (data.admissionNumber) {
               ...
               //  const transactionsResponse = await axios.get(
               //     `http://127.0.0.1:5000/api/students/studentTransactions/${data.admissionNumber}`
               //  );

               const transactionsResponse = await axios.get(
                  `https://lms-backend-0hls.onrender.com/api/students/studentTransactions/${data.admissionNumber}`
               );
   ...
   const updateFeeHandler = async () => {
      ...
      try {
         ...
         const { data } = await axios.put(
            'https://lms-backend-0hls.onrender.com/api/students/fees/nios',
            feeBody,
            config
         );
   ...
   if (studentData.length > 0) {
      ...
      if (mode === 'Correspondent') {
         feeOptions = feeOptions.filter((option) =>
            ['examFees', 'registrationFees', 'admissionFees'].includes(
               option.value
            )
         );
               ...
               <div className="h-screen pt-10 pb-4 px-5 overflow-hidden ">
                  <div className="h-full w-full  grid grid-rows-9 space-y-2">
                     <div className="row-span-1  flex justify-between">
                        <div>
                           <h1 className="text-xl md:text-2xl text-[#2740CD] font-bold ">
                              Update fee
                           </h1>
                           <p className="text-[#333333]">
                              Revise and Manage Student Fees
                           </p>
                        </div>
                        <div
                           className={`${
               ...
               <div className="col-span-6 h-full  px-12 grid grid-rows-9 3xl:grid-rows-12 overflow-hidden ">
                  ...
                  <div className="row-span-1 h-4/5   ">
                     {studentData.length > 0 ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center">
                           <h1 className="text-md font-normal text-[#333333]">
                              No student data available
                           </h1>
                        </div>
                     )}
              ...
              <div className="w-full p-2">
                ...
                <label for="button" className={`${student && student.feeDetails.installments[2].isPaid ? 'hidden' : 'block'} text-xs font-medium text-red-600 mt-2 text-center`}>{student && student.feeDetails.installments[2].paidAmount > 0 && `${student.feeDetails.installments[2].amount - student.feeDetails.installments[2].paidAmount} bal`}</label>
                  ...
                  <div className="row-span-4 pt-10">
                     <h1 className="text-lg lg:text-xl pb-2">
                        Recent Transactions
                     </h1>
                           ...
                           {transactions && transactions.length > 0 ? (
                              ...
                              <div>
                                 <img
                                    ...
                                    alt=""
                                 />
                                 <h1 className="text-center text-md font-normal lead text-[#333333]">
                                    No transactions available
                                 </h1>
                              </div>
                           )}
                           ...
                           {transactions && transactions.length > 0 ? (
                              ...
                              <div className=" xl:flex xl:flex-col justify-center items-center">
                                 <img
                                    ...
                                    alt=""
                                 />
                                 <h1 className="text-center text-md font-normal text-[#333333]">
                                    No transactions available
                                 </h1>
                              </div>
                           )}
