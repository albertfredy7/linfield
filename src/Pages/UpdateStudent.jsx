...
function UpdateStudent() {
  ...
  const selectedExamMonthOption = examMonthOptions.find(
    (option) => option.value === examMonth
  );
  ...
  const selectedExistingStudentOption = booleanOptions.find(
    (option) => option.value === existingStudent
  );

  const selectedtmaSubmittedOption = booleanOptions.find(
    (option) => option.value === String(tmaSubmitted)
  );
  const selectedtmaReceivedOption = booleanOptions.find(
    (option) => option.value === String(tmaReceived)
  );
  const selectedExamModeOption = examModeOptions.find(
    (option) => option.value === examMode
  );
            ...
            {studentData.length > 0 ? (
              <div className="row-span-10 flex flex-col gap-2  h-full overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div
                    className={`flex flex-col gap-2 pt-2 ${
                      course === 'SSLC' ? 'col-span-2' : 'col-span-1'
                    }`}
                  >
                    <label
                      htmlFor="course"
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div
                    className={` flex flex-col gap-2 pt-2 col-span-${
                      existingStudent === 'true' ? '1' : '2'
                    } pt-2`}
                  >
                    <label
                      htmlFor="existingStrudent"
