...
function Insights() {
   ...
   const formatNumber = (number) => {
      if (number >= 1000000) {
         return number / 1000000 + 'M'; // Convert to millions
      } else if (number >= 100000) {
         return number / 100000 + 'L'; // Convert to lakhs
      } else if (number >= 1000) {
         return number / 1000 + 'K'; // Convert to thousands
      } else {
         return number;
      }
   };
   ...
   const fetchInsightCategoryData = async () => {
      try {
         ...
         if (
            ...
            // apiUrl = `http://127.0.0.1:5000/api/general/insights?duration=${selectedDuration}&category=${selectedCategory}`;
            apiUrl = `https://lms-backend-0hls.onrender.com/api/general/insights?duration=${selectedDuration}&category=${selectedCategory}`;
         } else {
            ...
            // apiUrl = `http://127.0.0.1:5000/api/general/insights?duration=custom&start_date=${selectedDuration}&category=${selectedCategory}`;
            apiUrl = `https://lms-backend-0hls.onrender.com/api/general/insights?duration=custom&start_date=${selectedDuration}&category=${selectedCategory}`;
         }
         ...
         setInsightCategoryData(transformedData.reverse());
      } catch (error) {
   ...
   const fetchInsightsData = async () => {
      ...
      const { data } = await axios.get(
         'https://lms-backend-0hls.onrender.com/api/transactions/info'
      );
      setInsightData(data);
   };
   ...
   const handleAddRevenue = async () => {
      try {
         ...
         //  const { data } = await axios.post(
         //     'http://127.0.0.1:5000/api/transactions/addRevenue',
         //     revenueData
         //  );

         const { data } = await axios.post(
            'https://lms-backend-0hls.onrender.com/api/transactions/addRevenue',
            revenueData
         );
               ...
               <div className="flex flex-col h-screen">
                  <div className="px-5 pt-10">
                     ...
                     <p className="text-[#66666] text-nowrap">
                        Detailed Analysis of Institution Performance
                     </p>
                  </div>
                     ...
                     <div className=" h-full flex flex-row gap-2 items-center justify-center">
                        <div className=" h-full flex flex-col justify-center items-center  relative">
                           <p className="text-base 3xl:text-4xl text-[#2740CD] font-bold">
                              {insightData.admission &&
                                 formatNumber(
                                    insightData[selectedCategory].dailyData
                                 )}
                           </p>
                        ...
                        <div className=" h-full flex flex-col justify-center items-center  relative">
                           <p className="text-base 3xl:text-4xl text-[#2740CD] font-bold">
                              {insightData.admission &&
                                 formatNumber(
                                    insightData[selectedCategory].weeklyData
                                 )}
                           </p>
                           <p className="block md:hidden text-xs text-[#333333] text-nowrap">
                        ...
                        <div className="col-span-1 h-full flex flex-col justify-center items-center  relative">
                           <p className="text-base 3xl:text-4xl text-[#2740CD] font-bold">
                              {insightData.admission &&
                                 formatNumber(
                                    insightData[selectedCategory].monthlyData
                                 )}
                           </p>
                           <p className="block md:hidden text-xs text-[#333333] text-nowrap">
                  ...
                  >
                     {insightCategoryData.length > 0 ? (
                        insightCategoryData.slice().map((data, index) => {
                           return (
                              <DataCard
                                 type={
                        ...
                        <div className="text-center text-base font-semibold overflow-y-hidden flex flex-col justify-center items-center">
                           <img
                              ...
                              alt=""
                           />
                           <h1 className="text-center text-gray-500 text-base font-normal">
                              No data available
                           </h1>
                        </div>
                     )}
               ...
               <div className="col-span-6  overflow-hidden py-2 px-4">
                  <div className="w-full h-full  grid grid-rows-7 space-y-2 ">
                     ...
                     <div className="row-span-1 flex items-center px-2">
                        <Button
                           buttonStyle={
                              'p-4 lg:p-6 lg:px-8 bg-white rounded-lg'
                           }
                           text={'Add revenue'}
               ...
               <div className="col-span-9 grid grid-rows-5 3xl:grid-rows-6 pl-8 pr-2 overflow-hidden">
                  <div className="row-span-1 grid grid-cols-6 items-center pt-4 pb-7   px-4">
                     <div className="col-span-1 h-full flex flex-col justify-center">
                        <h2 className="text-xl 3xl:text-3xl font-semibold">
                           Insights
                        </h2>
